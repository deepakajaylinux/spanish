...
Una máquina virtual basada en el kernel (KVM ) es una infraestructura de virtualización integrada para el sistema operativo Linux y diseñado para funcionar con la arquitectura del procesador basado en x86 . Se compone de un módulo de kernel , kvm.ko , que proporciona la infraestructura de virtualización de núcleo y un módulo específico del procesador, kvm- intel.ko o kvm- amd.ko . KVM también requiere un QEMU modificado aunque se está trabajando para conseguir los cambios necesarios en sentido ascendente.

 KVM es un software de código abierto. El uso de KVM , se puede ejecutar múltiples máquinas virtuales que se ejecutan las imágenes sin modificar Linux o Windows . Cada máquina virtual tiene hardware virtualizado privado : una tarjeta de red , disco , adaptador de gráficos , etc.

Ayuda Comando
--------------

Este comando ayuda a determinar el uso de KVM. En él se enumeran los parámetros alternativos de KVM. También se describe la sintaxis
para el funcionamiento del módulo KVM. El comando de ayuda para kvm se muestra a continuación .
...
El comando utilizado para instalar el módulo de KVM en el terminal aparece a continuación ,
...
* Reducción de costes
* Enterprise Performance y escalabilidad superior
* Seguridad avanzada
